// Note: API endpoints /api/storefront-data, /api/coupons/validate, and /api/products/:id/decrease-stock will be implemented in Phase 2
    ...
    <>
      ...
      <div className="announcement-bar">
        <div className="announcement-track">
          {(() => {
            const getAnnText = (ann) => {
              if (!ann) return '';
              if (typeof ann === 'string') return ann;
              if (typeof ann.text === 'string') return ann.text;
              if (typeof ann.text === 'object' && ann.text) {
                return ann.text[i18n.language] || ann.text.ar || ann.text.en || '';
              }
              return '';
            };

            let validAnnouncements = (settings.announcements || []).filter(ann => getAnnText(ann).trim() !== '');
            ...
            // Repeat 4 times for seamless infinite marquee scroll across all screen widths
            const repeatedArr = [...validAnnouncements, ...validAnnouncements, ...validAnnouncements, ...validAnnouncements];
            return repeatedArr.map((ann, idx) => (
              <span key={idx}>
                <i className={ann.icon || 'fa-solid fa-star'}></i> {getAnnText(ann)}
              </span>
            ));
          })()}
        </div>
